...
5. **รายงานเดือน** — สรุปรายจ่ายจริงตามหมวดเทียบ cost model, **"เงินออกจากบัญชี" รวม 3 แหล่ง** (รายจ่ายปกติ + ซื้อทรัพย์สิน + จ่ายคืนหนี้ ที่มี `payment_account_id` ตรงกัน — เพื่อกระทบยอดบัญชีธนาคารได้ครบจริง ไม่ใช่แค่รายจ่ายปกติเหมือนก่อนหน้านี้), รายรับ-รายจ่ายรวม (**ไม่รวมค่าเสื่อม** — ดูข้อ 12), มูลค่าทรัพย์สินสุทธิ, หนี้สินคงค้างแยกตามเจ้าหนี้
...
ตัวเลข "คงเหลือโดยประมาณ" ในแท็บรายงานเดือนตอนนี้ = รายรับ (ยอดโอนเข้าจริง) − รายจ่ายดำเนินงาน (จาก `expense_items`) เท่านั้น — **ยังไม่ใช่กำไรสุทธิที่แท้จริงตามหลักบัญชี** เพราะขาดค่าเสื่อมของเดือนนั้นไป (ข้อดีคือมันก็ไม่ได้ลบเงินคืนหนี้ออกด้วย ซึ่งถูกต้องแล้ว — อธิบายเหตุผลด้านล่าง)

**สูตรกำไรสุทธิที่ถูกต้อง**:
```
กำไรสุทธิ = รายรับ − รายจ่ายดำเนินงาน − ค่าเสื่อมของเดือนนั้น
```
(ไม่ใช่ค่าเสื่อมสะสมทั้งหมด แค่ "ส่วนของเดือนนี้เดือนเดียว" = `purchase_price ÷ useful_life_months` ของทุกทรัพย์สินที่ active อยู่ในเดือนนั้น — ตอนนี้ระบบยังไม่มีเลขนี้แสดงในรายงาน มีแต่ "มูลค่าทรัพย์สินคงเหลือสุทธิ" ซึ่งเป็นยอดคงเหลือสะสม ณ วันนี้ ไม่ใช่ค่าเสื่อมเฉพาะเดือน)
...
**กำไร vs เงินสดในมือ คือคนละคำถามกัน** — เดือนไหนซื้อเครื่องใหม่ก้อนใหญ่หรือผ่อนคืนหนี้เยอะ "กำไร" อาจยังดูดีตามปกติ (เพราะทั้งสองอย่างไม่กระทบกำไร) แต่ "เงินสดในบัญชีจริง" จะหายไปเยอะเดือนนั้น — ทั้งสองตัวเลขถูกต้องพร้อมกันได้ ไม่ใช่ขัดแย้งกัน ต้องดูคู่กันเสมอ ไม่ใช่ดูแค่ตัวใดตัวหนึ่ง

**ยังไม่ได้ทำ** (แจ้งไว้ตอนตอบคำถามผู้ใช้): เพิ่มแถว "ค่าเสื่อมเดือนนี้" + "กำไรสุทธิ (รวมค่าเสื่อม)" ในตารางรายงานเดือน — คำนวณได้จากข้อมูลที่มีอยู่แล้วทั้งหมด (`assets.purchase_price/useful_life_months`, กรองเฉพาะทรัพย์สินที่ active ในเดือนนั้น) ไม่ต้องแก้ schema แค่เพิ่ม query+แสดงผล
