...
- [x] Phase 4: `products` + หน่วยแปลง + auto-link + เครื่องมือกวาดชื่อซ้ำ — build เสร็จ ทดสอบจริงผ่านหมด (auto-link ตอนบันทึก, bulk-link ชื่อที่ไม่ตรง, คำนวณราคาต่อหน่วยใช้งานถูกต้อง) **ยัง push**
- [x] เอกสาร architecture ถาวร (`CLAUDE.md`/`PROJECT_OVERVIEW.md`) — อัปเดตต่อเนื่องทุก phase

## Phase ถัดไป (ยังไม่เริ่ม)
- คำนวณต้นทุนต่อเมนู (recipe costing) — ต้องผูกกับ `menu_items` ของ CRM repo (`buddy-platform`), Phase 4 เตรียม "ราคาต่อหน่วยใช้งาน" ไว้ให้พร้อมแล้วแต่ยังไม่มีหน้าคำนวณสูตรอาหาร
