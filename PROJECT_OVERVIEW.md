...
2. **รายการรายจ่าย** — ledger/บัญชีแยกประเภท (ต่างจาก "รายงานเดือน" ที่เป็นสรุปภาพใหญ่): browse รายการรายจ่ายแบบรายไอเทม, ปุ่ม preset วันนี้/เดือนนี้/เดือนที่แล้ว/**เพิ่มล่าสุด** หรือกรองเองตามช่วงวันที่/หมวดหมู่/บัญชีที่จ่าย, จัดกลุ่มตามวันที่ — ปุ่ม **"เพิ่มล่าสุด"** เรียงตาม `expenses.created_at` (เวลาที่บันทึกจริง) ไม่ใช่ `expense_date` (วันที่ที่พิมพ์ไว้) ใช้หารายการที่เพิ่งบันทึกแต่จำวันที่ที่พิมพ์ไว้ไม่ได้ (เผื่อพิมพ์วันผิด) — ดูข้อ 11 เรื่องบั๊ก order-by-foreign-table ที่เจอตอนทำฟีเจอร์นี้ — แต่ละไอเทมมี **3 ปุ่มหลัก** เท่านั้น: "ดูรูปสลิป" (ถ้ามีรูปแนบ — เปิด signed URL ทุกรูปพร้อมกัน, โชว์ตัวเลขจำนวนรูปถ้ามากกว่า 1), "แก้ไข" (ขยายแผงแก้ไขใต้การ์ด), "ลบ" (ลบ item สุดท้ายของใบเสร็จแล้ว auto ลบ header ที่ว่างเปล่าทิ้งด้วย) — แผง **"แก้ไข"** รวม 4 ส่วนไว้ที่เดียว (เดิมกระจายเป็นปุ่มแยก 3-4 ปุ่ม ทำให้การ์ดรกและเคยเป็นต้นเหตุ layout ล้นมาแล้ว):
